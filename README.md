...
Все ключи обязательны для успешно загруженных страниц. Поле `error` у страницы и ресурса включается в JSON только когда ошибка непустая. На страницах с ошибкой загрузки `broken_links` и `assets` равны `null`.
